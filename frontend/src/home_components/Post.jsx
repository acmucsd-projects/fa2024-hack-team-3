...
const Post = ({ post, onDelete, onEdit }) => {
    ...
    const fetchComments = async () => {
        try {
            ...
            const response = await axios.get(`https::/localhost:5000/api/posts/${post._id}/comments`);
    ...
    const handleAddComment = async () => {
        ...
        try {
            const response = await axios.post(
                `https::/localhost:5000/api/posts/${post._id}/comments`,
                payload,
                { 
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(`https::/localhost:5000/api/posts/${post._id}`, {
                headers: {
    ...
    const handleSaveEdit = async () => {
        ...
        const updatedCourse =
            ...
            await axios.patch(
                `https::/localhost:5000/api/posts/${post._id}`,
                updatedPost,
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('authToken')}`, // Include token
            ...
            const response = await axios.get(`https::/localhost:5000/api/posts/${post._id}`);
    ...
    const handleEditComment = async (id, updatedText) => {
        ...
        try {
          const response = await axios.patch(
            `https::/localhost:5000/api/posts/comments/${id}`,
            { text: updatedText },
      ...
      const handleDeleteComment = async (id) => {
        try {
            await axios.delete(`https::/localhost:5000/api/posts/comments/${id}`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}` },
            });
